...
for(long object = objs_start; object<objs_end; object++){
	...
	//printf("   current_weight %d \n", current_weight); 
		...
		results[row_index] = MAX(check1,CurrentlyLastRow[row_index-current_weight]+current_profit);
		}
...
void solve_kp(long objs_start, long objs_end, long* weights, long* profits, long* results1, long* results2, long capacity, int* solutions, int firstcall)
{
...
long* results1_call1 = (long *)malloc((capacity_call1+2)*sizeof(long));
...
solve_kp(objs_start, objs_mid, weights, profits, results1_call1, results2_call1, capacity_call1, solutions, 0);




//printf(" objs end %d \n", objs_end);
solve_kp(objs_mid, objs_end, weights, profits, results1_call2, results2_call2, capacity_call2, solutions, 0);
...
solve_kp(0, N, weights, profits, results1, results2, C, solutions, 0);
