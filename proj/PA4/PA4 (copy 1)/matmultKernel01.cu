...
//this is where they go into memory
  int footprint_row = block_row;
  int footprint_col = block_col;
...
//4 different C values
  ...
  int bFact = BLOCK_SIZE*BLOCK_SIZE;
...
//top left and top right
	Cvalue0 += shared_A[thread_row*FOOTPRINT_SIZE + e]*shared_B[e*FOOTPRINT_SIZE + thread_col];
	Cvalue1 += shared_A[thread_row*FOOTPRINT_SIZE + e]*shared_B[e*FOOTPRINT_SIZE + thread_col+BLOCK_SIZE];
	Cvalue2 += shared_A[(thread_row+BLOCK_SIZE)*FOOTPRINT_SIZE + e]*shared_B[e*FOOTPRINT_SIZE + thread_col];
	Cvalue3 += shared_A[(thread_row+BLOCK_SIZE)*FOOTPRINT_SIZE + e]*shared_B[e*FOOTPRINT_SIZE + thread_col+BLOCK_SIZE];
