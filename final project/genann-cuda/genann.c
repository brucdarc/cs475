...
/*
    ...
    for (j = 0; j < ann->hidden; ++j) {
		double const *wt = w + ann->inputs * j;
        double sum = *wt++ * -1.0;		
        for (k = 0; k < ann->inputs; ++k) {
            sum += wt[k] * i[k];
        }		
        o[j] = genann_act_hidden(ann, sum);
    }
	w += ann->inputs * (ann->hidden+1);
	o += ann->hidden;
*/
...
*/
    ...
    for (h = 1; h < ann->hidden_layers; ++h) {
		...
		#pragma omp parallel for
        for (j = 0; j < ann->hidden; ++j) {
			double const *wt = w + (ann->hidden+1) * (j);

			//printf("w at start of loop %d \n", w);
            double sum = *wt++ * -1.0;
			//#pragma omp parallel for reduction (+:sum)
		    for (k = 0; k < ann->hidden; ++k) {
		        sum += wt[k] * i[k];
...
/*
		//#pragma omp parallel for
        for (j = 0; j < ann->outputs; ++j) {
			double const *wt = w + (ann->outputs+1) * (j);
			...
			wt += ann->hidden;
            o[j] = genann_act_output(ann, sum);
			//printf("w at end of loop %d \n", w);
        }
		w += (ann->output+1) * ann->hidden;
		o = o + ann->output;
*/
	...
	//printf(" w-ann: %f , ann totalw : %f \n", w - ann->weight, ann->total_weights);
...
*/
		...
		#pragma omp parallel for
        for (j = 0; j < ann->hidden; ++j) {
			double *wt = w + ((h == 0 ? ann->inputs : ann->hidden)+1) * j;

            *wt++ += d[j] * learning_rate * -1.0;
            for (k = 1; k < (h == 0 ? ann->inputs : ann->hidden) + 1; ++k) {
                *wt++ += d[j] * learning_rate * i[k-1];
            }
